const input = `rjq: mkj bbr xrh mnn
zch: zvf pnk bbk njb
jms: vhm gdp jxq tmj
qkz: qpl rlb rfv
lnl: knh nls
...
let nodes = lines.map((line) => {
    ...
    return { id: tokens[0], edges: tokens[1].split(" ") };
});
...
// add nodes not set eplicitely
for (let i = 0; i < nodes.length; i++){
    let node = nodes[i];
    for (let j = 0; j < node.edges.length; j++){
        let n2 = graph[node.edges[j]];
        if (!n2){
            n2 = { id: node.edges[j], edges: [] };
            graph[n2.id] = n2;
        }
    }
}
...
console.log(nodes.reduce((total, n) => total + n.edges.length, 0));

let edges = [];
for (let i = 0; i < nodes.length; i++){
    let node = nodes[i];
    for (let j = 0; j < node.edges.length; j++){
        edges.push([node.id, node.edges[j]]);
    }
}

// add second value of edges 
for (let i = 0; i < nodes.length; i++){
    let node = nodes[i];
    for (let j = 0; j < node.edges.length; j++){
        let n2 = graph[node.edges[j]];        
        if (!n2.edges.includes(node.id)){
            n2.edges.push(node.id);
        }
    }
}
...
function fordFulkerson(graph, startNode, endNode){

    let edgeFlow = edges.map((e) => {return { flow: 0, dir: 0 }})

    
}

let count = 0;
// ford fulkerson between node n1 (fixed) and n2 (variable) until finding maximum flow of 3 
let n1 = 0;
for (let n2 = 1; n2 < nodes.length; n2++){

    let result = fordFulkerson(graph, nodeIds[n1], nodeIds[n2]);
