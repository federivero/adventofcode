const input = `F.F77.F7F7FFJJ..77FF.--L777.-7777--F7F7FJFFF7FF|.F77FF|-7-|-F7-7-FF|FJ-FFJ7-F--F|-L--7L|-7FFFF---7-FJ.7J.F-7J.F|7-LF7L7|.F-L|7...LF--LL7F-77
...
let h = pipes.length;
let w = pipes[0].length;

let toVisit = [];
let distances = pipes.map((p, rIndex) => p.split("").map((t, cIndex) => { 
    if (t === 'S'){
        toVisit.push({ r: rIndex, c: cIndex });
        return 0;
    }else{
        return -1;
    }
...
while (toVisit.length > 0){
    let next = toVisit.splice(0, 1)[0];
    let pipe = pipes[next.r][next.c];
    let distance = distances[next.r][next.c] + 1;
    ...
    if (pipe === 'S'){
        ...
        if (next.r > 0){
            let r = next.r - 1;
            let c = next.c;
            if (['|','F', '7'].includes(pipes[r][c])){
                toVisit.push({ r: r, c: c });
        ...
        toTest.push({ r: next.r - 1, c: next.c })
        toTest.push({ r: next.r + 1, c: next.c })
    }else if (pipe === '-'){
        toTest.push({ r: next.r, c: next.c - 1 })
        toTest.push({ r: next.r, c: next.c + 1 })        
    }else if (pipe === 'F'){
        toTest.push({ r: next.r + 1, c: next.c })
        toTest.push({ r: next.r, c: next.c + 1 })                
        ...
        console.log("UNRECOGNIZED PIPE", pipe)
        return;
    }
    for (let p = 0; p < toTest.length; p++){
        let testPipe = toTest[p];
        if (testPipe.r >= 0 && testPipe.c >= 0 && testPipe.r < h && testPipe.c < w){
            if (distances[testPipe.r][testPipe.c] === -1){
                toVisit.push(testPipe)
                distances[testPipe.r][testPipe.c] = distance;
...
function isLoop(r, c){
    return loop.find((l) => l.r === r && l.c === c);
}

let insides = 0;
for (let r = 0; r < h; r++){
    let outside = true;
    let borderStart = null;
    for (let c = 0; c < w; c++){
        if (isLoop(r, c)){
            if (pipes[r][c] === '|'){
                outside = !outside;
...
let map = pipes.map((t) => t.split("").map((t2) => "."));
for (let i = 0; i < loop.length; i++){
    let l = loop[i];
    map[l.r][l.c] = pipes[l.r][l.c];
}
...
let dirMap = pipes.map((t) => t.split("").map((t2) => "."));
let searchMap = pipes.map((t) => t.split("").map((t2) => "."));
...
for (let i = 1; i < loop.length; i++){
    let current = loop[i];
    let pipeDir = getDirection(previous, current);
    ...
    switch (pipeDir) {
        ...
        case DIR_ENUM.UP:
            ...
            break;
        case DIR_ENUM.DOWN:
            dirMap[previous.r][previous.c] = 'V' 
            cStep = -1;
            break;
    }
    let finished = false;
    ...
    while (!finished){
        ...
        if (newR >= 0 && newC >= 0 && newR < h && newC < w){
            let isBorder = loop.find((l) => l.r === newR && l.c === newC);
            if (isBorder){
                finished = true;
            } else {
                if (!insides.find((l) => l.r === newR && l.c === newC)){
                    insides.push({ r: newR, c: newC })
                    map[newR][newC] = 'I'
                }
            }
        }else{
...
// console.log(dirMap.map((l) => l.join("")));
*/
