const input = `bg gcdaeb aebg efabdcg abdce cafdbe fcbdeg bdacg gbd cafgd | daecb dcbae gb eabg
...
for (let i = 0; i < lines.length; i++){
    ...
    for (let d = 0; d < startPart.length; d++){
        let st = startPart[d];
        if (!row.wordPerCount[st.length]) row.wordPerCount[st.length] = []
        row.wordPerCount[st.length].push(st);
    }

    // real 'a' segment is the extra one between 7 and 1  
    const one = row.wordPerCount[2][0].split("")
    const seven = row.wordPerCount[3][0].split("")

    const a = seven.filter((s) => !one.includes(s))[0]
    row.segments.a = a;

    // real 'bd' segments are the extra ones between 4 and 1  
    const four = row.wordPerCount[4][0].split("")
    const bd = four.filter((s) => !one.includes(s))

    // real 'e', 'c' and 'd' sements are the ones missing between 8 and [9, 6, 0] respectively
    const ecd = [];
    const eight = row.wordPerCount[7][0].split("")
    for (let q = 0; q < row.wordPerCount[6].length; q++){
        let num = row.wordPerCount[6][q].split("")
        let segment = eight.filter((s) => !num.includes(s))[0]
        ecd.push(segment);
    }

    // d segment is the overlap between ecd and bd
    const d = bd.filter((s) => ecd.includes(s))[0]
    row.segments.d = d;

    // b segment is the one which is not d in 'bd'
    const b = bd.filter((s) => d !== s)[0]
    row.segments.b = b;

    // find ec segments
    const ec = ecd.filter((s) => s !== d)

    // find f segment (the segments in 4 which are not ec or bd)
    const f = four.filter((s) => !ec.includes(s) && !bd.includes(s))[0]
    row.segments.f = f;

    // find c segment (the segment in 1 which is not f)
    const c = one.filter((s) => s !== f)[0]
    row.segments.c = c;  

    // find e segment (segment in ec which is not c)
    const e = ec.filter((s) => s !== c)[0]
    row.segments.e = e;

    // find g segment (segment in eight which has not been found yet)
    let foundLetters = Object.values(row.segments).filter((s) => s !== '');
    const g = eight.filter((s) => !foundLetters.includes(s))[0]
    row.segments.g = g;

    // invert the translation
    let keys = Object.keys(row.segments);
    for (let k = 0; k < keys.length; k++){
        let value = row.segments[keys[k]];
        row.inverted[value] = keys[k];
    }

    let finalpart = parts[1];
    let digits = finalpart.trim().split(" ");
    let num = 0;
    for (let d = 0; d < digits.length; d++){
        let digit = digits[d].split("");
        let translated = digit.map((d) => row.inverted[d])
        translated.sort()
        translated = translated.join("")
        
        let number = sevenSegmentsMap[translated]
        if (number === undefined){
            console.log("DIDN'T FIND", number)
        }else{
            num = num * 10 + number;
        }
    }
