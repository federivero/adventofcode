const input = `hcpjssql4kjhbcqzkvr2fivebpllzqbkhg
...
let lines = input.replaceAll("one", "onee").
    replaceAll("two", "twoo").
    replaceAll("eight", "eightt").
    replaceAll("three", "threee").
    replaceAll("nine", "ninee").
    replaceAll("five", "fivee").
    replaceAll("seven", "sevenn").split("\n")

let sum = 0;
for (let i = 0; i < lines.length; i++){
    let line = lines[i];
    const digits = line.match(/(\d|one|two|three|four|five|six|seven|eight|nine)/g).map((d) => {
        const n = parseInt(d)
        if (isNaN(n)){
            switch (d){
                case 'one': return 1;
    ...
    const d2 = parseInt(digits[digits.length - 1])
    sum += d1 * 10 + d2;
    console.log(d1, d2);
}
