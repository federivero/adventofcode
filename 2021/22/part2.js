const input = `on x=-14..32,y=-22..28,z=-25..26
...
const sample = `on x=-5..47,y=-31..22,z=-19..33
...
const sampleSmall = `on x=-20..26,y=-36..17,z=-47..7
...
const sampleTiny = `on x=-2..7,y=-2..7,z=-2..7
on x=-2..7,y=-2..7,z=-2..7
on x=-2..7,y=-2..7,z=-2..7
off x=-2..7,y=-2..7,z=-2..7`
...
commands = commands.map((c) => {
	...
	let comm = {};
	comm.type = c[0] === 'on'? 1 : -1;

	let axis = c[1].split(",");
	axis[0] = axis[0].substring(2);
	const x1 = parseInt(axis[0].split("..")[0]);
	const x2 = parseInt(axis[0].split("..")[1]);
    comm.x1 = Math.min(x1, x2);
    comm.x2 = Math.max(x1, x2);

	axis[1] = axis[1].substring(2);
	const y1 = parseInt(axis[1].split("..")[0]);
	const y2 = parseInt(axis[1].split("..")[1]);
    comm.y1 = Math.min(y1, y2);
    comm.y2 = Math.max(y1, y2);

	axis[2] = axis[2].substring(2);
	const z1 = parseInt(axis[2].split("..")[0]);
	const z2 = parseInt(axis[2].split("..")[1]);
    comm.z1 = Math.min(z1, z2);
    comm.z2 = Math.max(z1, z2);
...
Octtree.prototype.addEnclosedEntry = function(type, x, y, z, childIndex){
    this.leaf = false;
    this.x = x;
    this.y = y;
    this.z = z;
    this.children = [];
    for (let i = 0; i < 8; i++){
        this.children.push(new Octtree());
    }
    this.children[childIndex].value = type;
}

Octtree.prototype.addCommand = function(comm){
    if (this.leaf){
        this.leaf = false;
        this.x = comm.x1;
        this.y = comm.y1;
        ...
        this.children = [];
        for (let i = 0; i < 8; i++){
            this.children.push(new Octtree(this.value));
        }
        this.children[7].addEnclosedEntry(comm.type, comm.x2, comm.y2, comm.z2, 0);
    }else{
        let index = 0;
        if (comm.x1 < this.x && this.x < comm.x2){
            // need to split x axis
            let comm1 = {...comm};
            let comm2 = {...comm};
            comm1.x2 = this.x;
            comm2.x1 = this.x;
            this.addCommand(comm1);
            this.addCommand(comm2);
            return;
        }else if (this.x <= comm.x1){
            index += 1;
        }
        if (comm.y1 < this.y && this.y < comm.y2){
            // need to split y axis
            let comm1 = {...comm};
            let comm2 = {...comm};
            comm1.y2 = this.y;
            comm2.y1 = this.y;
            ...
            this.addCommand(comm2);
            return;
        }else if (this.y <= comm.y1){
            index += 2;
        }
        if (comm.z1 < this.z && this.z < comm.z2){
            // need to split y axis
            let comm1 = {...comm};
            let comm2 = {...comm};
            comm1.z2 = this.z;
            ...
            this.addCommand(comm2);
            return;
        }else if (this.z <= comm.z1){
            index += 4;
        }
        this.children[index].addCommand(comm);
    } 
}

Octtree.prototype.calculateTotal = function(minx, miny, minz, maxx, maxy, maxz){
    let total = 0;
    if (this.leaf){
        if (this.value === 1){
            console.log("adding", minx, maxx, miny, maxy, minz, maxz, "---", (maxx - minx + 1) * (maxy - miny + 1) * (maxz - minz + 1))
        ...
        for (let i = 0; i < 8; i++){
            ...
            let newmaxz = maxz;
            if ((i & 1) === 0){
                newmaxx = Math.min(this.x, maxx);
            }else{
                newminx = Math.max(this.x, minx);
            }
            if ((i & 2) === 0){
                newmaxy = Math.min(this.y, maxy);
            }else{
                newminy = Math.max(this.y, miny);                
            }
            if ((i & 4) === 0){
                newmaxz = Math.min(this.z, maxz);
            }else{
                newminz = Math.max(this.z, minz);
            }
            let childrenTotal = this.children[i].calculateTotal(newminx, newminy, newminz, newmaxx, newmaxy, newmaxz);
            total += childrenTotal;
        }
        return total;
    }
...
for (let i = 0; i < 2; i++){
    let comm = commands[i];
    tree.addCommand(comm); 
    console.log(tree.calculateTotal(-Infinity, -Infinity, -Infinity, Infinity, Infinity, Infinity));   
}
*/
...
for (let i = 0; i < commands.length; i++){
    ...
    for (let j = 0; j < cubes.length; j++){
        ...
        let x1 = Math.max(cube.x1, other.x1);
        let x2 = Math.min(cube.x2, other.x2);
        let y1 = Math.max(cube.y1, other.y1);
        let y2 = Math.min(cube.y2, other.y2);
        let z1 = Math.max(cube.z1, other.z1);
        let z2 = Math.min(cube.z2, other.z2);

        if ((x2 - x1) >= 0 && (y2 - y1) >= 0 && (z2-z1) >= 0){
            let sign = 1;
            if (cube.type === 1 && other.type === 1){
                sign = -1;
            }else if (cube.type === 1 && other.type === -1){
                sign = 1;
            }else if (cube.type === -1 && other.type === 1){
                sign = -1;
            }else if (cube.type === -1 && other.type === -1){
            ...
            let newcube = { x1, x2, y1, y2, z1, z2, type: sign }
...
for (let i = 0; i < cubes.length; i++){
    let c = cubes[i]
    let add = c.type * (c.x2 - c.x1 + 1) * (c.y2 - c.y1 + 1) * (c.z2 - c.z1 + 1)
    sum += add;
}
console.log(sum);
...
// console.log(tree.children[6].children[7])
// console.log(tree.calculateTotal(-Infinity, -Infinity, -Infinity, Infinity, Infinity, Infinity));