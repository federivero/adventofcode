...
function sendPulses(pulseList, module, pulseType, pulseNumber){0
    for (let i = 0; i < module.destinations.length; i++){
        if (printPulses){
            console.log(`${module.id} -${pulseType ? 'high' : 'low'}-> ${module.destinations[i]}`)
        }
        if (module.destinations[i] === 'rx' && pulseType === PULSE_TYPE_ENUM.LOW){
            console.log(`pulse sent to RX ${pulseNumber}`);
        }
...
let initialPulseCount = 100 * million;
        ...
        if (module.type === TYPE_ENUM.FLIP_FLOP){
            if (pulse.type === PULSE_TYPE_ENUM.LOW){
                if (module.state === FLIP_FLOP_TYPE_ENUM.OFF){
                    ...
                    sendPulses(pulsesSending, module, PULSE_TYPE_ENUM.HIGH, i)
                }else{
                    module.state = FLIP_FLOP_TYPE_ENUM.OFF;                    
                    sendPulses(pulsesSending, module, PULSE_TYPE_ENUM.LOW, i)
                }
            } // else, it's ignored
        } else if (module.type === TYPE_ENUM.CONJUCTION){
            module.receivedPulses[pulse.origin] = pulse.type;
            let hasLowPulses = module.inputPulses.some((mod) => module.receivedPulses[mod.id] === PULSE_TYPE_ENUM.LOW);
            if (module.id === 'hp'){
                let hasHighPulses = module.inputPulses.reduce((count, mod) => module.receivedPulses[mod.id] === PULSE_TYPE_ENUM.HIGH ? count + 1 : count, 0);
                if (hasHighPulses > 1){
                    console.log(i, module.inputPulses.map((mod) => `${mod.id}: ${module.receivedPulses[mod.id]}`).join(" - "))
                }
            }
            if (hasLowPulses){
                sendPulses(pulsesSending, module, PULSE_TYPE_ENUM.HIGH, i);
