...
var clone = function(number){
	if (isNumber(number)){
		return number;
	}else{
		return { left: clone(number.left), right: clone(number.right) }; 
	}
}
...
for (let i = 0; i < numbers.length; i++){
	...
	for (let j = 0; j < numbers.length; j++){
		if (i != j){
			let num2 = numbers[j];
			let sum = { left: clone(num1), right: clone(num2)};
			let result = reduce(sum);
			let mag = magnitude(result); 
			maxMagnitude = Math.max(mag, maxMagnitude);
			let sum2 = { left: clone(num2), right: clone(num1) };
			result = reduce(sum2);
			mag = magnitude(result); 
			maxMagnitude = Math.max(mag, maxMagnitude);
			// console.log(i, j, maxMagnitude);
		}
	}
}
