...
for (let i = 0; i < backpacks.length; i++){
    ...
    for (let j = 0; j < bp.length; j++){
        ...
        if (!badges[item])
            badges[item] = 0;
        badges[item] = badges[item] | (1 << (i % 3));
    }
    if (i % 3 === 2){
        let items = Object.keys(badges);
        for (let j = 0; j < items.length; j++){
            let item = items[j]
            if (badges[item] === 7){
                if (item.charCodeAt(0) >= 'a'.charCodeAt(0)){
                    totalPriorities += item.charCodeAt(0) - 'a'.charCodeAt(0) + 1;
                }else{
                    totalPriorities += item.charCodeAt(0) - 'A'.charCodeAt(0) + 27;
                }
                break;
            }
        }        
