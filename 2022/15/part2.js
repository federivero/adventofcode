...
for (let q = 0; q < size; q++){
    ...
    for (let i = 0; i < sensors.length; i++){
        ...
        if (distleft > 0){
            spaces[q].push([s.sx - distleft, s.sx + distleft]);
        }
    }    
}
...
for (let i = 0; i < spaces.length; i++){
    let line = spaces[i];
    line.sort((s1, s2) => s1[0] - s2[0]);
    if (line.length > 0){
        let min = line[0][0];
        let max = line[0][1];
        if (min > 0){
            console.log("found ", 0, i); return;
        }
        for (let l = 1; l < line.length; l++){
            let g = line[l];
            if (max + 1 < g[0]){
                console.log("found! ", max + 1, i, ((max+1)* size + i));
            }
            min = g[0];
            max = Math.max(g[1], max);
        }    
    }
}
